...
function prev() {
    ...
    ipcRenderer.send('group:prev', Code);
}

function nxt() {
    btnNo = 1;
    var Code = document.querySelector('#input1').value;
    if (Code == "") ipcRenderer.send('group:fst');
    else ipcRenderer.send('group:nxt', Code);
}

function fst() {
    btnNo = 2;
    ipcRenderer.send('group:fst');
}

function lst() {
    btnNo = 2;
    ipcRenderer.send('group:lst');
}
...
function show() {
    ...
    else ipcRenderer.send('group:data', Code);
}

ipcRenderer.on('group:getData', (e, arg) => {
    if (arg != null) {
        let arr = ['Code', 'GroupName', 'HoLoc'];
...
function submit() {
    if (document.querySelector('#input0').files[0] != undefined) {
        ...
        btnSet();
        ipcRenderer.send('group:submit', path);
    }
    else {
        let arr = [];
        for(let i=1; i<=3; i++) arr.push(document.querySelector(`#input${i}`).value);
        
        if (arr[0] == "" || arr[1] == "") dialog.showMessageBox({ type: "error", message: 'Required fields cannot be empty!' })
        else {
            btnSet();
            ipcRenderer.send('group:submit', arr);
        }
    }
}

ipcRenderer.on('group:dataAdded', (event, arg) => {
    if (arg != null) dialog.showMessageBox({ type: "info", message: arg })
    window.location.reload();
});

function prt() {
    ipcRenderer.send('group:report');
}

ipcRenderer.on('group:report:made', (event, arg) => {
